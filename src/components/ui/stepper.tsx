...
function Stepper({
  ...
  const [stepperDef] = useState(() => Stepperize.defineStepper(steps));
...
function StepperTrigger({
  ...
  const { state, isLoading, step, isDisabled, index } = useStepItem();
  const {
    stepper,
    registerTrigger,
    focusNext,
  ...
  // Step and trigger order are kept in sync by StepperItem.
  const myIdx = index;
...
function StepperIndicator({
  ...
  const base =
    "relative flex size-8 shrink-0 items-center justify-center overflow-hidden rounded-md border text-sm font-medium transition-all duration-300";

  const defaultClasses = cn(
    "border-slate-300 bg-white text-slate-600 data-[state=completed]:border-blue-600 data-[state=completed]:bg-blue-600 data-[state=completed]:text-white data-[state=active]:border-blue-600 data-[state=active]:bg-blue-600 data-[state=active]:text-white",
    base
  );
...
function StepperTitle({ children, className }: React.ComponentProps<"h3">) {
  ...
  return (
    <h3
      ...
      data-state={state}
      className={cn(
        "text-sm font-medium text-slate-700 data-[state=active]:text-blue-600 data-[state=completed]:text-blue-600",
        className
      )}
    >
      {children}
